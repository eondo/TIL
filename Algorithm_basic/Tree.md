...
1. __편향 이진트리(Skewed Binary Tree)__
...
- 왼쪽에서 return해서 온 거면, 왼쪽 다 순회했다는 거니까 해당 부모 루트를 방문 처리함
...
[참고] 이진 트리의 저장 - **포화/완전 이진트리가 아닌 경우**
...
- 중위 순회하면 오름차순으로 정렬된 값을 얻을 수 있음
  - 🚫 __Caution__ : 루트값을 잘못 설정하면, 루트의 왼오 중 한쪽만 깊어지는 구조가 되어 트리 구조에서 비효율적
...
❓ 탐색 연산 vs 순회
차이점 : 순회는 재귀 형태로 되돌아오고~, 탐색 연산은 굳이 재귀를 할 필요 없음. 둘의 한 쪽만 선택해서 가고, 찾거나 실패하면 끝내면 되고 굳이 리턴해서 같은 경로로 돌아올 필요가 없기 때문. 트리에서의 탐색이 꼭 재귀일 필요 없고, 반복 구조로도 구현 가능함.

- 검색 알고리즘의 비교, 이러한 차이점이 있다 정도만 알고 넘어감
- 상용에서 검색을 위해 어떤 알고리즘을 사용할까 -> ? 넘어감

### 3. 삭제 연산
- 12 삭제하는 경우
  - 9에서 12로 내려가고, 타고 15까지 내려갔을 때 9와 15을 잇고 12를 날림
- 9 삭제하는 경우가 까다로움 
  - 왼쪽 서브트리에서 가장 큰 값을 찾아서 그것을 9의 원래 자리에 옮겨놓고 원래의 왼쪽 최대값을 날림
<br>

## 힙(heap)
[참고] 연결리스트 쓸 때, 프로그램이 실행되는 중에 사용하는 메모리의 양, 크기가 변할 수 있는 경우에 사용하는 메모리 공간을 힙이라고도 함.
...
- 23 삽입
2) 조건에서 부모가 더 작아지는 상황 발생 -> 자리 바꿈 -> 조건이 다 만족할 때까지 자리 바꿈
