...
오픈제플린을 통해 ERC-721 기반 프로젝트를 구현할 수 있다. 프로젝트에서 쓰이는 NFT 발급(민팅)과 스마트 컨트랙트를 구현이 필요하다.
...
#### ERC-NFT 기능 관련 대표적 함수

- ERC-721
  - `balanceOf` : 오너가 가진 토큰 수 반환
  - `ownerOf` : 토큰의 오너 주소 반환
  - `approve` : 토큰에 요청을 보낸 계정에 대하여 전송 승인
  - `getApproved` : 해당 토큰의 전송이 승인된 주소 반환
  - `transferFrom` : from -> to 주소로 토큰 전송
  - `safeTransferFrom` : from -> to 주소로 토큰 전송. 호출자가 from이 아닐 경우 approve/setApproveForAll을 통해 토큰 전송을 허가받아야 하는 제약 존재
  - `_mint` : 토큰 민팅
  - `_transfer` : 토큰 전송
- NFT
  - `mint` : 토큰을 민팅
  - `TockenTracking` : 토큰의 거래 내역 반환
